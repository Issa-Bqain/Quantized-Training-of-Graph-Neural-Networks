...
def run_NC_DQ_training_loop(accuracy: list, DQ: bool, DQB: list, losses: list=[]):
    ...
    class QGCN(torch.nn.Module):
        def __init__(self, hidden_channels):
            ...
            self.conv1 = quantised_gcn_layer.bothGCNConv(dataset.num_features, hidden_channels)
            self.conv2 = quantised_gcn_layer.bothGCNConv(hidden_channels, dataset.num_classes)
    ...
    class GAT(torch.nn.Module):
        def __init__(self, in_channels, hidden_channels, out_channels, heads):
            super().__init__()
            self.conv1 = quantised_gat_layer.CustomGATConv(in_channels, hidden_channels, heads, dropout=0.6)
            # On the Pubmed dataset, use `heads` output heads in `conv2`.
            self.conv2 = quantised_gat_layer.CustomGATConv(hidden_channels * heads, out_channels, heads=1,
    ...
    for epoch in range(1, 151):
        ...
        if epoch % 1 == 0:
            #print(f'Epoch: {epoch:03d}, Loss: {loss:.4f}, Current Avg = {current_avg:.4f}, Previous Avg = {previous_avg:.4f}')
            print(f'Epoch: {epoch:03d}, Loss: {loss:.4f}')
